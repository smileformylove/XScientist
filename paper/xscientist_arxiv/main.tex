...
\node[state] (root) {root\\question};
\node[good, right=of root] (exp) {experiment\\node};
\node[warn, above right=of exp] (fail) {failed\\branch};
\node[good, right=of fail] (repair) {repair\\rerun};
\node[good, below right=of exp] (ablate) {ablation\\node};
\node[paper, right=2.25cm of exp] (candidate) {manuscript\\candidate};
\node[state, right=of candidate] (claim) {claimref\\anchor};
\node[state, below=of candidate] (fork) {fork\\seed};
...
repository commit, configuration, model choices, output directory, and the ARA
root used for any generated paper.
