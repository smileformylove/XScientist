...
def _render_bfts_config(profile: str, model: str) -> str:
    ...
    if execution_is_glm53:
        required = "__xscientist_non_glm_judgment_model_required__"
        for key in ("feedback", "vlm_feedback", "summary"):
            section = agent.get(key)
            if section is None:
                section = {"temp": 0.2, "max_tokens": 4096}
                agent[key] = section
            if not isinstance(section, dict):
                raise WorkspaceInitError(
                    f"packaged BFTS profile {profile!r} has invalid agent.{key}"
                )
            section["model"] = required
        judgment = agent.get("judgment")
        if judgment is None:
            judgment = {"temp": 0.2, "max_tokens": 4096}
            agent["judgment"] = judgment
        if not isinstance(judgment, dict):
            raise WorkspaceInitError(
                f"packaged BFTS profile {profile!r} has invalid agent.judgment"
            )
        judgment["model"] = required
        ...
        if not isinstance(select_node, dict):
            raise WorkspaceInitError(
                f"packaged BFTS profile {profile!r} has invalid agent.select_node"
            )
        select_node["model"] = required
    else:
        for key in ("feedback", "vlm_feedback", "summary"):
            section = agent.get(key)
            if section is None:
...
def create_workspace(
    ...
    *,
    ...
    try:
        originals = {
            relative: _capture_workspace_file_state(root / relative)
    ...
    written_post_states: dict[str, _WorkspaceFileState] = {}
    pending_write: str | None = None

    def leaf_still_matches_snapshot(relative: str) -> bool:
        target = root / relative
        try:
            return _capture_workspace_file_state(target) == originals[relative]
        except OSError:
            return False
    ...
    try:
        ...
        for relative, content in files.items():
            ...
            if relative == ".gitignore" and target.exists():
                try:
                    content = _merged_workspace_gitignore(
                        originals[relative].content.decode("utf-8"),
                        content,
                    )
            ...
            pending_write = relative
            atomic_write_text(target, content)
            try:
                post_state = _capture_workspace_file_state(target)
            except OSError as write_exc:
                raise WorkspaceInitError(
                    f"could not verify managed workspace file after writing: {relative}"
                ) from write_exc
            expected_content = content.encode("utf-8")
            if post_state is None or post_state.content != expected_content:
                raise WorkspaceInitError(
                    f"managed workspace file changed concurrently: {relative}"
                )
            written_post_states[relative] = post_state
            pending_write = None
        ...
        if pending_write is not None:
            try:
                pending_matches_original = (
                    _capture_workspace_file_state(root / pending_write)
                    == originals[pending_write]
                )
            except OSError:
                pending_matches_original = False
            if not pending_matches_original:
                # The writer raised before an invocation-owned post-state could be
                # captured. Preserve the ambiguous leaf instead of guessing.
                rollback_failed = True
        try:
            if (
