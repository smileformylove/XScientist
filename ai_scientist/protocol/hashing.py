...
def hash_node_payload(
    *,
    ...
    ``llm_call_hashes`` optionally binds semantic LLM call-receipt hashes
    that produced this node's code (``call_receipt_ref.hash`` from new traces;
    legacy traces may use ``messages_ref.hash`` from
    ``<ara>/llm/calls.jsonl``). When supplied, two nodes with identical code
    and metric but generated by different prompts, outputs, models, or semantic
    parameters hash differently. Order-insensitive:
