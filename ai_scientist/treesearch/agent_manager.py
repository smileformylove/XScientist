...
"""
    ...
    def _check_stage_completion(self, stage: Stage) -> bool:
        ...
        if stage.stage_number == 2:
            ...
            if len(set(tested_datasets)) < 2:
                missing_msg = (
                    "Stage 2 requires results on at least two datasets; "
                    f"currently have {len(set(tested_datasets))}: {tested_datasets}"
                )
                logger.info(missing_msg)
                print(f"[yellow]{missing_msg}[/yellow]")
        ...
        if stage.stage_number == 3:
            ...
            if len(set(tested_datasets)) < 3:
                missing_msg = (
                    "Stage 3 expects broader evidence (>=3 datasets total); "
                    f"currently have {len(set(tested_datasets))}: {tested_datasets}"
                )
                logger.info(missing_msg)
                print(f"[yellow]{missing_msg}[/yellow]")
    ...
    def _get_best_implementation(self, stage_name: str) -> Optional[Node]:
        ...
        if stage_name not in self.journals:
            return None
        best_node = self.journals[stage_name].get_best_node_by_metric()
        if best_node:
