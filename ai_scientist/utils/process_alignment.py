...
def _safe_quality_result(project_root: Path) -> dict[str, Any]:
    payload = load_json_artifact(
        project_root / "quality" / "high_quality_result.json", default={}
    )
...
def build_process_alignment(project_root: str | Path) -> dict[str, Any]:
    ...
    research_plan = (
        load_contract_artifact(resolved_root, "research_plan", default={}) or {}
    )
    claim_graph = (
        load_contract_artifact(resolved_root, "claim_evidence_graph", default={}) or {}
    )
    experiment_records = load_jsonl_artifact(
        artifact_path(resolved_root, "experiment_registry")
    )
    figure_spec = load_contract_artifact(resolved_root, "figure_spec", default={}) or {}
    manuscript_state = (
        load_contract_artifact(resolved_root, "manuscript_state", default={}) or {}
    )
    review_state = (
        load_contract_artifact(resolved_root, "review_state", default={}) or {}
    )
    repair_plan = load_contract_artifact(resolved_root, "repair_plan", default={}) or {}
    self_evolution = (
        load_contract_artifact(resolved_root, "self_evolution", default={}) or {}
    )
    ...
    graph_nodes = [
        item for item in (claim_graph.get("nodes") or []) if isinstance(item, dict)
    ]
    graph_edges = [
        item for item in (claim_graph.get("edges") or []) if isinstance(item, dict)
    ]
    figure_rows = [
        ...
        if process == "ideation":
            ...
            by_process[process] = _finalize_process(
                ...
                [
                    ...
                    _criterion(
                        ...
                        passed=bool(
                            str(lead_idea.get("core_hypothesis") or "").strip()
                        ),
                    ),
                    _criterion(
                        "novelty_claim",
                        "Lead idea declares novelty or related-work positioning",
                        passed=bool(
                            str(lead_idea.get("novelty_claim") or "").strip()
                            or str(lead_idea.get("related_work_notes") or "").strip()
                        ),
                    ),
                    _criterion(
                        "minimum_experiment",
                        ...
                        passed=bool(
                            str(
                                lead_idea.get("minimum_viable_experiment") or ""
                            ).strip()
                        ),
                    ),
                    _criterion(
                        "datasets_metrics_baselines",
                        "Lead idea lists datasets, metrics, and baselines",
                        passed=bool(lead_idea.get("candidate_datasets"))
                        and bool(lead_idea.get("candidate_metrics"))
                        and bool(lead_idea.get("candidate_baselines")),
                    ),
                    _criterion(
                        "failure_criteria",
                        "Lead idea includes explicit failure criteria",
                        passed=bool(lead_idea.get("failure_criteria")),
                    ),
                ],
                signals={
                ...
                risks=(
                    []
                    if lead_idea.get("failure_criteria")
                    else ["idea_failure_criteria_gap"]
                ),
            )
        elif process == "program":
            budget = (
                research_plan.get("budget")
                if isinstance(research_plan.get("budget"), dict)
                else {}
            )
            execution_policy = (
            ...
            by_process[process] = _finalize_process(
                ...
                [
                    _criterion(
                        "research_program",
                        "Research program markdown exists",
                        passed=bool(research_program_text.strip()),
                    ),
                    _criterion(
                        "budget_fields",
                        "Research plan declares max steps, wallclock, and retry budget",
                        passed=all(
                            isinstance(budget.get(key), int)
                            for key in (
                                "max_steps",
                    ...
                    _criterion(
                        "acceptance_rules",
                        "Execution policy declares acceptance rules",
                        passed=bool(execution_policy.get("acceptance_rules")),
                    ),
                    _criterion(
                        "registry_expectations",
                        "Execution policy declares registry discipline",
                        passed=bool(execution_policy.get("registry_expectations")),
                        required=False,
                    ),
                    _criterion(
                        "success_criteria",
                        "Research program names success criteria",
                        passed="## Success Criteria" in research_program_text,
                    ),
                    _criterion(
                        "failure_handling_rules",
                        "Research program names failure handling rules",
                        passed="## Failure Handling" in research_program_text,
                    ),
                ],
                signals={
                ...
                risks=(
                    []
                    if execution_policy.get("acceptance_rules")
                    else ["program_acceptance_rule_gap"]
                ),
            )
            ...
            by_process[process] = _finalize_process(
                ...
                [
                    _criterion(
                        "task_count",
                        "Research plan includes explicit tasks",
                        passed=bool(tasks),
                        detail=f"count={len(tasks)}",
                    ),
                    _criterion(
                        "claim_targets",
                        "Tasks target downstream claims",
                        passed=(
                            all(bool(item.get("claim_targets")) for item in tasks)
                            if tasks
                            else False
                        ),
                    ),
                    _criterion(
                        "graph_nodes",
                        "Claim graph contains explicit nodes",
                        passed=bool(graph_nodes),
                    ),
                    _criterion(
                        "graph_edges",
                        "Claim graph contains explicit edges",
                        passed=bool(graph_edges),
                    ),
                    _criterion(
                        "expected_outputs",
                        "Tasks declare expected outputs",
                        passed=(
                            all(bool(item.get("expected_outputs")) for item in tasks)
                            if tasks
                            else False
                        ),
                    ),
                    _criterion(
                        "task_kinds",
                        "Exploration tracks task kinds for agentic branching",
                        passed=bool(task_kinds),
                        required=False,
                    ),
                ],
                signals={
            ...
            by_process[process] = _finalize_process(
                ...
                [
                    _criterion(
                        ...
                        passed=bool(experiment_records),
                        detail=f"count={len(experiment_records)}",
                    ),
                    _criterion(
                        "budget_status",
                        "Every experiment record reports budget status",
                        passed=all(
                            str(item.get("budget_status") or "").strip()
                            for item in experiment_records
                        ),
                    ),
                    _criterion(
                        "acceptance_checks",
                        "Every experiment record carries acceptance checks",
                        passed=all(
                            bool(item.get("acceptance_checks"))
                            for item in experiment_records
                        ),
                    ),
                    _criterion(
                        "completed_or_failed",
                        "Experiment registry preserves terminal records",
                        passed=bool(completed_records)
                        or any(
                            str(item.get("status") or "").strip() == "failed"
                            for item in experiment_records
                        ),
                    ),
                    _criterion(
                        "result_summary",
                        "Completed records keep structured result summaries",
                        passed=(
                            all(
                    ...
                    _criterion(
                        "storyline_trace",
                        "At least one record is explicitly promoted into the storyline",
                        passed=bool(storyline_records),
                        required=False,
                    ),
                ],
                signals={
            ...
            if not figure_rows:
                ...
                continue
            workflow_mode = (
                str(research_plan.get("workflow_mode") or "").strip().lower()
            )
            ...
            expected_main_ready = (
                ...
                if int(figure_summary.get("claim_count") or 0)
                else min(
            ...
            by_process[process] = _finalize_process(
                ...
                [
                    _criterion(
                        "figure_count",
                        "Figure spec contains figures",
                        passed=bool(figure_rows),
                        detail=f"count={len(figure_rows)}",
                    ),
                    _criterion(
                        "claim_binding",
                        "Each figure is bound to a claim",
                        passed=all(
                    ...
                    _criterion(
                        "data_traceability",
                        "Each figure cites data files and source records",
                        passed=all(
                            bool(item.get("data_files"))
                            and bool(item.get("source_records"))
                            for item in figure_rows
                        ),
                    ),
                    _criterion(
                        "caption_intent",
                        "Each figure defines caption intent",
                        passed=all(
                            str(item.get("caption_intent") or "").strip()
                            for item in figure_rows
                        ),
                    ),
                    _criterion(
                        ...
                        passed=float(figure_summary.get("claim_coverage_ratio") or 0.0)
                        >= 1.0,
                    ...
                    _criterion(
                        ...
                        or int(figure_summary.get("main_ready_count") or 0)
                        >= expected_main_ready,
                        required=strict_visual_lane
                        or bool(figure_summary.get("claim_count")),
                        detail=(
                    ...
                    _criterion(
                        ...
                        passed=(
                            ...
                            or int(
                                figure_summary.get("ready_missing_data_file_count") or 0
                            )
                            == 0
                        ),
                        required=bool(
                            figure_summary.get("checked_data_file_availability")
                        ),
                        detail=(
                    ...
                    _criterion(
                        "ready_figures",
                        "At least one figure is ready for use",
                        passed=bool(ready_figures),
                    ),
                ],
                signals={
            ...
            by_process[process] = _finalize_process(
                ...
                [
                    _criterion(
                        "outline",
                        "Manuscript state includes an outline",
                        passed=bool(manuscript_state.get("outline")),
                    ),
                    _criterion(
                        "section_briefs",
                        "Manuscript state includes section briefs",
                        passed=bool(section_briefs),
                    ),
                    _criterion(
                        "claim_bindings",
                        "Manuscript state binds claims",
                        passed=bool(manuscript_state.get("claim_bindings")),
                    ),
                    _criterion(
                        "figure_bindings",
                        "Manuscript state binds figures or tables",
                        passed=bool(manuscript_state.get("figure_bindings"))
                        or bool(manuscript_state.get("table_bindings")),
                    ),
                    _criterion(
                        "guardrail_status",
                        "Writing guardrail status is not blocked",
                        passed=str(
                            manuscript_state.get("guardrail_status") or ""
                        ).strip()
                        not in {"", "blocked"},
                    ),
                    _criterion(
                        "missing_evidence",
                        "Writing state is not missing evidence",
                        passed=not manuscript_state.get("missing_evidence"),
                        required=False,
                    ),
                ],
                signals={
                    ...
                    "missing_evidence_count": len(
                        _coerce_list(manuscript_state.get("missing_evidence"))
                    ),
                },
                risks=(
                    ["writing_missing_evidence"]
                    if manuscript_state.get("missing_evidence")
                    else []
                ),
            )
        elif process == "review":
            ...
            by_process[process] = _finalize_process(
                ...
                [
                    _criterion(
                        ...
                        passed=bool(review_state.get("rounds")),
                    ),
                    _criterion(
                        "role_coverage",
                        "Review covers multiple reviewer roles",
                        passed=float(review_metrics.get("role_coverage_ratio") or 0.0)
                        >= 0.5,
                        required=False,
                        detail=f"role_coverage_ratio={float(review_metrics.get('role_coverage_ratio') or 0.0):.2f}",
                    ),
                    _criterion(
                        "binding_coverage",
                        "Review issues are bound to claim / figure / section targets",
                        passed=float(
                            review_metrics.get("target_binding_coverage") or 0.0
                        )
                        >= 1.0,
                    ),
                    _criterion(
                        "repair_queue",
                        "Active issues are converted into repair queue items",
                        passed=float(review_metrics.get("repair_queue_coverage") or 0.0)
                        >= 1.0,
                    ),
                    _criterion(
                        "repair_ready",
                        "Repair queue is verification-ready",
                        passed=float(review_metrics.get("repair_ready_coverage") or 0.0)
                        >= 1.0,
                    ),
                    _criterion(
                        "repair_plan",
                        "Repair plan exists and tracks lane-oriented execution",
                        passed=bool(repair_plan_summary.get("task_count")),
                    ),
                ],
                signals={
                    "active_issue_count": int(
                        review_metrics.get("active_issue_count") or 0
                    ),
                    "repair_queue_count": int(
                        review_metrics.get("repair_queue_count") or 0
                    ),
                    "repair_ready_coverage": float(
                        review_metrics.get("repair_ready_coverage") or 0.0
                    ),
                },
                risks=(
                    ["review_binding_gap"]
                    if float(review_metrics.get("target_binding_coverage") or 0.0) < 1.0
                    else []
                ),
            )
            ...
            by_process[process] = _finalize_process(
                ...
                [
                    _criterion(
                        "self_check",
                        "Self-evolution self-check exists",
                        passed=bool(self_evolution_self_check),
                    ),
                    _criterion(
                        "required_failures",
                        "Self-evolution has no required failures",
                        passed=not self_evolution_self_check.get("required_failures"),
                    ),
                    _criterion(
                        "lessons",
                        "Self-evolution records lessons",
                        passed=bool(self_evolution.get("lessons")),
                    ),
                    _criterion(
                        "next_cycle_defaults",
                        "Self-evolution proposes next-cycle defaults",
                        passed=bool(self_evolution.get("next_cycle_defaults")),
                    ),
                    _criterion(
                        "score",
                        "Self-evolution score clears 80",
                        passed=float(self_evolution_summary.get("score") or 0.0)
                        >= 80.0,
                    ),
                    _criterion(
                        "stage_risks",
                        "Self-evolution surfaces stage risks",
                        passed=bool(self_evolution.get("stage_risks")),
                        required=False,
                    ),
                ],
                signals={
            ...
            by_process[process] = _finalize_process(
                ...
                [
                    _criterion(
                        "quality_result",
                        "Quality pipeline produced a successful result",
                        passed=str(quality_result.get("status") or "").strip()
                        == "success",
                    ),
                    _criterion(
                        "quality_gate",
                        "Submission-quality gate passed",
                        passed=quality_result.get("quality_gate_passed") is True,
                    ),
                    _criterion(
                        "submission_priority",
                        "Submission priority is present",
                        passed=isinstance(
                    ...
                    _criterion(
                        "submission_readiness",
                        "Submission readiness payload exists",
                        passed=bool(submission_readiness),
                    ),
                    _criterion(
                        "ready_or_rewrite",
                        "Run is either ready or has rewrite trace evidence",
                        passed=bool(submission_readiness)
                        and (
                            bool(submission_readiness.get("ready"))
                            or bool(quality_result.get("rewrite_trace"))
                        ),
                    ),
                    _criterion(
                        "packaging_assets",
                        "At least one submission-facing packaging asset exists",
                        passed=any(str(item or "").strip() for item in packaging_files),
                        required=False,
                    ),
                ],
                signals={
                ...
                risks=(
                    []
                    if quality_result.get("quality_gate_passed")
                    else ["submission_packaging_not_ready"]
                ),
            )
    ...
    attention_count = sum(
        item["status"] == "needs_attention" for item in process_results
    )
