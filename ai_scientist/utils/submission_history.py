...
def summarize_historical_submission_outcomes(
    ...
    *,
    ...
    for quality_path in quality_paths:
        ...
        if not result:
            continue
        venue = result.get("target_venue") or DEFAULT_TARGET_VENUE
    ...
    blocker_counts = [
        ...
        for item in entries
        if isinstance(item.get("blocker_count"), int)
    ]
    ...
    return {
        ...
        "avg_blockers": (
            (sum(blocker_counts) / len(blocker_counts)) if blocker_counts else None
        ),
...
def recommend_rewrite_focus_adjustments(
    ...
    *,
    ...
    summary = summarize_historical_submission_outcomes(
        research_root, target_venue=target_venue
    )
    ...
    if entries < 3:
        focus["rationale"].append(
            "insufficient historical data for section-level rewrite guidance"
        )
        return focus
    ...
    if category_rates.get("rigor", 0) >= 2:
        ...
        add_section(
            "method",
            "历史结果显示 rigor 问题频繁，优先补实验设定、baseline、消融与显著性细节。",
        )
        add_section(
            "results", "历史结果显示 rigor 问题频繁，优先补关键对比、误差条和统计解释。"
        )
    ...
    if category_rates.get("claim", 0) >= 2:
        ...
        add_section(
            "abstract",
            "历史结果显示 claim 支撑不足，优先把主 claim 绑定到具体数字和图表。",
        )
        add_section(
            "introduction",
            "历史结果显示 claim 支撑不足，收紧 contribution 表述，避免过度承诺。",
        )
    ...
    if category_rates.get("numeric", 0) >= 2 or category_rates.get("evidence", 0) >= 2:
        ...
        add_section(
            "abstract",
            "历史结果显示数字/证据覆盖不足，优先把 strongest numbers 放入摘要。",
        )
        add_section(
            "results",
            "历史结果显示数字/证据覆盖不足，优先把 strongest figure/table 和 key results 写清。",
        )
        add_section(
            "discussion",
            "历史结果显示数字/证据覆盖不足，解释 strongest result 的意义与边界。",
        )
        focus["rationale"].append(
            "numeric and evidence blockers frequently hurt submission outcomes"
        )
    if category_rates.get("quality", 0) >= 2:
        ...
        add_section(
            "abstract", "历史结果显示整体质量问题频繁，摘要需更凝练、更像成熟投稿稿。"
        )
        add_section(
            "introduction", "历史结果显示整体质量问题频繁，引言需更快进入问题与贡献。"
        )
        focus["rationale"].append(
            "overall quality blockers frequently hurt submission outcomes"
        )
    if category_rates.get("contribution", 0) >= 1:
        ...
        add_section(
            "introduction",
            "历史结果显示 contribution framing 偏弱，明确 contribution bullets 和对应证据。",
        )
        add_section(
            "conclusion",
            "历史结果显示 contribution framing 偏弱，总结最重要贡献与 strongest evidence。",
        )
        focus["rationale"].append(
            "contribution framing often remains too weak historically"
        )
    if target_venue == "nature" and category_rates.get("breakthrough", 0) >= 1:
        ...
        add_section(
            "introduction",
            "Nature 历史失败显示 broad significance 不足，需更清楚交代重大问题与跨领域意义。",
        )
        add_section(
            "discussion",
            "Nature 历史失败显示意义表达不足，需解释 strongest result 为什么重要。",
        )
        focus["rationale"].append(
            "breakthrough framing remains a recurring Nature-style weakness"
        )

    if not focus["rationale"]:
        focus["rationale"].append(
            "historical blocker patterns are stable; no extra section emphasis needed"
        )
        ...
        for round_item in result.get("rewrite_trace", []):
            for item in round_item.get("frontmatter", []):
                ...
                chosen = next(
                    (
                        ...
                        if candidate.get("index") == best_index
                    ),
                    None,
                )
                if not chosen:
                ...
                preference["frontmatter"][style_key] = (
                    preference["frontmatter"].get(style_key, 0.0) + weight
                )
            ...
            for item in round_item.get("targets", []):
                ...
                if not chosen:
                    continue
                style_key = _style_key_from_variant(chosen.get("style_variant"))
                preference["sections"][style_key] = (
                    preference["sections"].get(style_key, 0.0) + weight
                )
...
def recommend_rewrite_style_preferences(
    ...
    *,
    ...
    summary = summarize_historical_rewrite_preferences(
        research_root, target_venue=target_venue
    )
    ...
    def ordered(area: str) -> list[str]:
        ...
        if not scores:
            return (
                ["professional", "conservative", "assertive"]
                if area == "frontmatter"
                else ["conservative", "professional", "assertive"]
            )
        return [
            item[0]
            for item in sorted(scores.items(), key=lambda pair: pair[1], reverse=True)
        ]
    ...
    *,
    ...
    for quality_path in quality_paths:
        ...
        for round_item in result.get("rewrite_trace", []):
            ...
            if not isinstance(pre_priority, (int, float)) or not isinstance(
                post_priority, (int, float)
            ):
                continue
            ...
            for item in round_item.get("frontmatter", []):
                ...
                bucket = summary["frontmatter"].setdefault(
                    ...
                    {
                        "count": 0,
                        "priority_delta_total": 0.0,
                        "quality_delta_total": 0.0,
                        "weight_total": 0.0,
                    },
                )
                bucket["count"] += 1
            ...
            for item in round_item.get("targets", []):
                ...
                style_key = _style_key_from_variant(
                    summary_meta.get("selected_style_variant")
                )
...
def recommend_rewrite_efficiency_controls(
    ...
    *,
    ...
    summary = summarize_historical_rewrite_effectiveness(
        research_root, target_venue=target_venue
    )
    section_scores = []
    ...
    preferred_sections = [
        name
        for name, delta, count in sorted(
            section_scores, key=lambda item: item[1], reverse=True
        )
        if count >= 2 and delta > 0.5
    ][:4]
    deprioritized_sections = [
        name
        for name, delta, count in sorted(section_scores, key=lambda item: item[1])
        if count >= 2 and delta <= 0
    ][:3]

    rounds = summary.get("rounds") or {}
    round_count = rounds.get("count", 0)
    avg_round_priority_delta = (
        (rounds.get("priority_delta_total", 0.0) / round_count) if round_count else 0.0
    )
    ...
    if preferred_sections:
        rationale.append(
            f"historically effective sections: {', '.join(preferred_sections)}"
        )
    if deprioritized_sections:
        rationale.append(
            f"historically low-yield sections: {', '.join(deprioritized_sections)}"
        )
    if rewrite_round_adjustment > 0:
        ...
        rationale.append(
            "historical extra rewrite rounds often show limited priority uplift"
        )
    if not rationale:
        rationale.append(
            "historical rewrite efficiency is stable; keep default targeting"
        )
...
def recommend_reviewer_risk_mitigation(
    ...
    *,
    ...
    def add_blocker_objection(prefix: str):
        for item in recent_sample:
            for blocker in item.get("blockers", []):
                ...
                if (
                    prefix in normalized.lower()
                    and len(result["anticipated_objections"]) < 6
                ):
    ...
    if category_rates.get("rigor", 0) >= 2:
        add(
            "anticipated_objections",
            "Are the baselines, ablations, and statistical tests strong enough to support the claimed improvement?",
        )
        add(
            "claim_softening_advice",
            "Avoid robustness or superiority claims unless they are directly backed by ablations, significance tests, or strong baseline comparisons.",
        )
        add(
            "limitation_emphasis",
            "State clearly if robustness, sensitivity, or broader baseline coverage remains incomplete.",
        )
        add(
            "rebuttal_focus",
            "Prepare a concise defense built around baselines, ablations, significance, and implementation details.",
        )
    ...
    if category_rates.get("claim", 0) >= 2:
        add(
            "anticipated_objections",
            "Which exact figure, table, or quantitative result supports each main claim?",
        )
        add(
            "claim_softening_advice",
            "Prefer restrained wording such as 'suggests' or 'is consistent with' when evidence is incomplete.",
        )
        add(
            "limitation_emphasis",
            "Explicitly call out where evidence is promising but not yet definitive.",
        )
        add(
            "rebuttal_focus",
            "Map each major claim to one strongest result and one quantitative takeaway.",
        )
    ...
    if category_rates.get("numeric", 0) >= 2 or category_rates.get("evidence", 0) >= 2:
        add(
            "anticipated_objections",
            "Can the authors quantify the gain more precisely and tie it to the strongest visual/table evidence?",
        )
        add(
            "claim_softening_advice",
            "Avoid saying 'significant improvement' without naming the strongest numbers or comparison points.",
        )
        add(
            "limitation_emphasis",
            "Acknowledge where the evidence package is still thin or where visuals are not yet comprehensive.",
        )
        add(
            "rebuttal_focus",
            "Lead with the strongest numerical result, then point to the corresponding figure/table and comparison baseline.",
        )
    ...
    if category_rates.get("contribution", 0) >= 1:
        add(
            "anticipated_objections",
            "What is the concrete contribution beyond a narrower benchmark improvement or engineering tweak?",
        )
        add(
            "claim_softening_advice",
            "Do not overstate novelty when the contribution is mainly empirical or integrative.",
        )
        add(
            "limitation_emphasis",
            "Clarify what the method does not yet establish as a general principle.",
        )
        add(
            "rebuttal_focus",
            "State the contribution in one sentence, then connect it to one strongest result and one limitation.",
        )
        add("rationale", "contribution-framing objections recur historically")
    if category_rates.get("quality", 0) >= 2:
        add(
            "anticipated_objections",
            "Is the manuscript polished and focused enough for a competitive submission, or is the story still diffuse?",
        )
        add(
            "claim_softening_advice",
            "Prefer sharper, narrower claims over broad but weakly supported framing.",
        )
        add(
            "limitation_emphasis",
            "Acknowledge when the current draft prioritizes a narrow validated contribution over a broader speculative story.",
        )
        add(
            "rebuttal_focus",
            "Defend the narrative by identifying the one central claim and the strongest evidence chain behind it.",
        )
        add("rationale", "overall manuscript-quality objections recur historically")
    if target_venue == "nature" and category_rates.get("breakthrough", 0) >= 1:
        add(
            "anticipated_objections",
            "Why is this a broad scientific advance rather than a strong but incremental technical result?",
        )
        add(
            "claim_softening_advice",
            "Use breakthrough framing cautiously unless broad significance is strongly evidenced.",
        )
        add(
            "limitation_emphasis",
            "State clearly where broad significance remains a forward-looking interpretation rather than a demonstrated fact.",
        )
        add(
            "rebuttal_focus",
            "Prepare a broad-significance defense grounded in the problem importance and strongest evidence, while keeping caveats explicit.",
        )
        add(
            "rationale", "Nature-style broad-significance objections recur historically"
        )

    if not result["anticipated_objections"]:
        add(
            "anticipated_objections",
            "What is the strongest empirical or conceptual takeaway of this paper?",
        )
        add(
            "rebuttal_focus",
            "Anchor every response in one strongest result and one explicit limitation.",
        )
...
def recommend_section_reviewer_language_guidance(
    ...
    *,
    ...
    section = (section_name or "general").lower()
    risks = recommend_reviewer_risk_mitigation(
        target_venue, research_root=research_root
    )
    ...
    if any(key in section for key in ["title", "abstract"]):
        ...
        add(
            "recommended_tone",
            "Make the strongest numerical takeaway explicit if available.",
        )
        add(
            "claim_softening",
            (
    ...
    if "introduction" in section:
        add(
            "recommended_tone",
            "Frame novelty narrowly and connect it to a concrete contribution list.",
        )
        add(
            "claim_softening",
            (
        ...
        add(
            ...
            (
                limitation_emphasis[0]
                if limitation_emphasis
                else "Clarify the intended scope of the contribution early."
            ),
        )
        add(
            "objection_preemption",
            (
                rebuttal_focus[0]
                if rebuttal_focus
                else "Anticipate the strongest novelty/significance objection in the motivation paragraph."
            ),
        )
    if any(key in section for key in ["results", "experiment", "analysis"]):
        ...
        add(
            ...
            (
                claim_softening[0]
                if claim_softening
                else "Avoid saying 'significant improvement' without concrete numbers."
            ),
        )
        add(
            "objection_preemption",
            (
                rebuttal_focus[0]
                if rebuttal_focus
                else "Preempt reviewer concern by naming the exact strongest evidence item."
            ),
        )
    if any(key in section for key in ["discussion", "conclusion"]):
        ...
        add(
            ...
            (
                rebuttal_focus[0]
                if rebuttal_focus
                else "Address the most likely reviewer concern before the final takeaway sentence."
            ),
        )
    if "method" in section:
        ...
        add(
            ...
            (
                claim_softening[0]
                if claim_softening
                else "Do not imply universality if experiments only cover a narrow setting."
            ),
        )
    ...
    if not any(guidance.values()):
        add(
            "recommended_tone",
            "Keep the language evidence-backed, concrete, and modest.",
        )
    if objections:
...
def recommend_submission_strategy_adjustments(
    ...
    *,
    ...
    venue = target_venue or DEFAULT_TARGET_VENUE
    venue_config = VENUE_PRESETS.get(venue, VENUE_PRESETS[DEFAULT_TARGET_VENUE])
    summary = summarize_historical_submission_outcomes(
        research_root, target_venue=venue
    )
    ...
    if entries < 3 or acceptance_rate is None:
        adjustments["rationale"].append(
            "insufficient historical data; using venue defaults"
        )
        return adjustments
    ...
    if acceptance_rate < 0.25:
        ...
        adjustments["rationale"].append(
            "historical acceptance is low; increasing rewrite and review depth"
        )
    elif acceptance_rate < 0.45:
        adjustments["writeup_retry_boost"] += 1
        adjustments["rationale"].append(
            "historical acceptance is middling; adding one extra writeup retry"
        )
    ...
    if category_rates.get("rigor", 0) >= 2:
        ...
        adjustments["review_ensemble_boost"] += 1
        adjustments["rationale"].append(
            "rigor blockers are common historically; increasing review depth and ensemble"
        )
    if category_rates.get("claim", 0) >= 2:
        adjustments["review_fewshot_boost"] += 1
        adjustments["rationale"].append(
            "claim blockers are common historically; increasing few-shot review guidance"
        )
    if category_rates.get("numeric", 0) >= 2 or category_rates.get("evidence", 0) >= 2:
        adjustments["cite_round_boost"] += 2
        adjustments["writeup_retry_boost"] += 1
        adjustments["rationale"].append(
            "numeric/evidence blockers are common historically; increasing citation and writeup budget"
        )
    if category_rates.get("quality", 0) >= 2:
        adjustments["writeup_retry_boost"] += 1
        adjustments["rationale"].append(
            "quality blockers are common historically; allowing one more polish pass"
        )
    if venue == "nature" and acceptance_rate < 0.5:
        adjustments["review_ensemble_boost"] += 1
        adjustments["review_reflection_boost"] += 1
        adjustments["rationale"].append(
            "Nature-style bar remains hard historically; tightening review process"
        )
    ...
    adjustments["review_reflection_boost"] = min(
        adjustments["review_reflection_boost"], 2
    )
    ...
    if not adjustments["rationale"]:
        adjustments["rationale"].append(
            "historical outcomes are stable; venue defaults look sufficient"
        )

    adjustments["effective_defaults"] = {
        "num_cite_rounds": venue_config.get("min_cite_rounds", 20)
        + adjustments["cite_round_boost"],
        "writeup_retries": venue_config.get("min_writeup_retries", 4)
        + adjustments["writeup_retry_boost"],
        "review_reflections": venue_config.get("min_review_reflections", 2)
        + adjustments["review_reflection_boost"],
        "review_ensemble": venue_config.get("min_review_ensemble", 3)
        + adjustments["review_ensemble_boost"],
        "review_fewshot": venue_config.get("min_review_fewshot", 2)
        + adjustments["review_fewshot_boost"],
    }
    return adjustments
