...
from ai_scientist.apps.daemon_reports import (
    _build_autonomy_program_markdown,
    _build_cycle_summary_markdown,
    _build_daily_report_markdown,
    _build_failure_guard_markdown,
    _build_handoff_report_markdown,
    _build_operator_brief_markdown,
    _build_primary_action_queue_markdown,
    _build_report_archive_index_markdown,
    _build_report_archive_trends_markdown,
    _build_source_batch_plan_markdown,
    _build_source_health_board_markdown,
    _build_source_next_batch_markdown,
    _build_source_runtime_board_markdown,
    _build_submission_autopilot_markdown,
)
from ai_scientist.apps.daemon_control import (
    _append_control_event,
    _apply_control_expiry,
    _default_control_payload,
    _ensure_control_file,
    _expiry_cycle_from_relative,
    _load_control_payload,
    _load_recent_control_events,
    _save_control_payload,
    _validate_control_payload,
)
from ai_scientist.apps.daemon_sources import (
    _coerce_one_or_many_str,
    _coerce_str_list,
    _load_source_config,
    _maybe_bool,
...
def _derive_evidence_gap_actions(
    ...
    if (
        ...
        and float(paper.get("evidence_density_score")) < 2.0
    ):
        actions.append(
    ...
    required_actions = [
        {
            ...
            "action": action,
            "reason": str(
                policy.get("reason") or "self-review gate indicates evidence gaps"
            ),
        }
        for idx, action in enumerate(evidence_gap_actions[:4])
    ]
    notes = (
        _coerce_str_list(paper.get("self_review_round_gate_reasons"))
        + evidence_gap_actions[:3]
    )
    focus = {
        ...
        "anticipated_objections": _coerce_str_list(paper.get("self_review_next_focus"))[
            :3
        ],
...
def _apply_pipeline_contract_feedback(
    ...
    review_strategy = str(getattr(parsed, "pipeline_contract_review_strategy", "depth"))
    updated = list(_clean_generator_args(passthrough))
    ...
    pressure = any(
        [
            ...
            int(summary.get("failed_experiment_count") or 0) >= failed_experiment_floor,
...
def _derive_rewrite_followup_policy(
    ...
    policy = {
        ...
        "auto_improvement_fallback": (execution_policy.allow_auto_improvement_fallback),
    ...
    if self_review_gate_ready is False and (
        ...
        or (
            ...
            and self_review_high_value_coverage < self_review_high_value_floor
        )
...
def _classify_source_runtime(
    ...
    if desired_policy == "program_driven":
        ...
        if isinstance(paper_types, list) and any(
            ...
            workflow_alignment_score += 1
        workflow_alignment_reason = "Review-board hardening prefers reviewer-facing sources with stricter submission posture."
        ...
        if isinstance(paper_types, list) and any(
            ...
            workflow_alignment_score += 1
        workflow_alignment_reason = "Multi-agent board prefers submission-facing sources that can support quality gate, hostile critic, and repair ownership."
    elif desired_policy == "writing_studio":
        if isinstance(paper_types, list) and any(
            str(item) in {"journal", "extended"} for item in paper_types
        ):
            workflow_alignment_score += 2
        workflow_alignment_reason = "Writing-studio preference favors sources that can turn evidence into polished long-form writeups."
    elif desired_policy == "classic_pipeline":
        if source_plan.get("source_archetype") in {"template_first", "adaptive"}:
            workflow_alignment_score += 1
        workflow_alignment_reason = "Classic pipeline prefers stable, repeatable sources with lower orchestration friction."
...
def _build_source_batch_plan_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for idx, row in enumerate(ready[:5]):
        if idx == 0:
            ...
            tier = "hold-ready"
            recommendation = f"Leave {row.get('name')} ready, but let higher-alignment sources run first."
        plan_rows.append(
            {
                "tier": tier,
                "source": row.get("name"),
...
def _hydrate_source_next_batch_advisory(
    ...
    for slot in hydrated.get("slots") or []:
        item = dict(slot)
        match = runtime_by_key.get(
            str(item.get("source_key") or "")
        ) or runtime_by_name.get(str(item.get("source") or ""))
        if match:
...
def _build_pipeline_contract_summary(manager: ResearchManager) -> dict[str, Any]:
    ...
    for row in pipeline_rows:
        ...
        if isinstance(review_repair_ready_coverage, (int, float)):
            ...
            if (
                ...
                and float(review_repair_ready_coverage) < 0.7
            ):
    ...
    for row in experiment_rows:
        ...
        policy_name = str(
            row.get("policy_name") or row.get("workflow_mode") or "unknown"
        )
    ...
    return {
        ...
        "execution_policy_counts": dict(
            sorted(
                execution_policy_counts.items(), key=lambda item: (-item[1], item[0])
            )
        ),
...
def _apply_auto_source_plan(
    ...
    def _build_mix_candidates() -> list[dict[str, Any]]:
        ...
        existing_action_sources = {
            ...
            if item.get("control_operation")
            in {"source-force-next", "source-boost-next"}
            and item.get("source")
        }
        ...
        if "mix_too_narrow" in recommendation_labels and dominant_archetype:
            ...
            if alternatives:
                ...
                existing_mix_sources = {
                    str(item.get("source")) for item in candidates if item.get("source")
                }
                if (
                    source_name
