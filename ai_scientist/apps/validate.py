...
def is_source_checkout() -> bool:
    return (PROJECT_ROOT / ".git").is_dir() and (
...
def run_installed_package_smoke() -> None:
    ...
    if not required_manifest_fields.issubset(manifest_schema.get("required") or []):
...
def run_helper_smoke() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with (
            ...
            mock.patch.object(launcher_workflow, "gather_citations", return_value=""),
            mock.patch.object(
                launcher_workflow,
                "perform_writeup",
        ...
        def _fake_get_batch_dir(
            batch_name: str, output_root: str | Path | None = None
        ) -> Path:
...
""",
                    ...
                    "sources": [
                        {
                            ...
                            "type": "topic",
                            "value": str(
                                PROJECT_ROOT / "examples" / "example_topic.md"
                            ),
                            "priority": 10,
        ...
        schema = json.loads(
            (
                PROJECT_ROOT / "configs" / "daemon" / "daemon_control.schema.json"
            ).read_text(encoding="utf-8")
        )
        assert schema.get("title") == "XScientist Daemon Control"
        source_schema = json.loads(
            (
                PROJECT_ROOT / "configs" / "sources" / "source_queue.schema.json"
            ).read_text(encoding="utf-8")
        )
        assert source_schema.get("title") == "XScientist Source Queue"
        daemon_profile_schema = json.loads(
            (
                PROJECT_ROOT / "configs" / "daemon" / "daemon_profile.schema.json"
            ).read_text(encoding="utf-8")
        )
        assert daemon_profile_schema.get("title") == "XScientist Daemon Profile"
        assert (
            PROJECT_ROOT / "configs" / "sources" / "stable_source_priority.example.json"
        ).exists()
        assert (
            PROJECT_ROOT
            / "configs"
            / "daemon"
            / "stable_daemon_profile.local.example.json"
        ).exists()
        ...
        stable_profile_run = subprocess.run(
            [
                ...
                str(
                    ...
                    / "configs"
                    / "daemon"
                    / "stable_daemon_profile.example.json"
                ),
                "--dry-run",
                "--print-command",
            ],
        ...
        stable_day_run = subprocess.run(
            [
                ...
                str(
                    ...
                    / "configs"
                    / "daemon"
                    / "stable_day_daemon_profile.example.json"
                ),
                "--dry-run",
                "--print-command",
            ],
        ...
        stable_night_run = subprocess.run(
            [
                ...
                str(
                    ...
                    / "configs"
                    / "daemon"
                    / "stable_night_daemon_profile.example.json"
                ),
                "--dry-run",
                "--print-command",
            ],
        ...
        assert (
            str(
                ...
                / "configs"
                / "sources"
                / "stable_source_priority.example.json"
            )
