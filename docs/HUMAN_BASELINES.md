...
| [PaperQA2 / LitQA2](https://arxiv.org/abs/2409.13740) (`measured_human_small_sample`) | Nine biology or related-science PhD/PhD-student evaluators answered assigned LitQA2 questions (248-question literature-retrieval set; 266 unique human answers) in two 20-question quiz rounds, with roughly one week per quiz, unrestricted institutional web/paper access, and an instruction not to use AI tools (not technically enforceable). | Human precision **73.8% ± 9.6%** and accuracy **67.7% ± 11.9%** (mean ± SD, *n*=9). | A direct same-benchmark literature-retrieval reference, not a human execution or end-to-end discovery score. The generous/variable time window, tool access, incentive scheme, and small evaluator sample limit comparability; the source does not report a matched human paper-writing or experiment arm. |
| [LAB-Bench](https://arxiv.org/abs/2407.10362) ([dataset](https://huggingface.co/datasets/futurehouse/lab-bench)) (`measured_human_small_sample`) | PhD-level biology researchers answered sampled quizzes from 2,457 practical biology MCQs; quiz size 20–140, **5–8 days** with no other time limit, internet/day-to-day research tools and ApE sequence software allowed, and an unenforced no-AI instruction. The audited text does not state a participant count; human coverage was uneven (69% overall: 100% LitQA2/SuppQA/FigQA/ProtocolQA/Cloning, 82% TableQA, 35% DbQA, 64% SeqQA). | Reported human accuracy/precision/coverage: LitQA2 **0.70/0.76/0.92**, SuppQA **0.85/0.86/0.98**, FigQA **0.77/0.82/0.94**, TableQA **0.84/0.87/0.96**, ProtocolQA **0.79/0.87/0.91**, CloningScenarios **0.60/0.73/0.82**. DbQA/SeqQA are reported by subtask; no single all-suite score is claimed. | Direct practical-biology capability reference, not experiment execution or end-to-end discovery. Humans had tools while model runs did not; coverage and expertise vary, and the paper warns the CloningScenarios human baseline may be low because questions can take 10–60 minutes. |
...
## Research-ideation human arm (separate from execution)

Idea generation is an important exploration signal, but it is not equivalent
to implementing, validating, or publishing a research result. The following
study is therefore kept in its own section rather than being treated as an
end-to-end scientific-research baseline.

| Study / primary source | Human arm and controls | Reported human result | Comparability boundary |
| --- | --- | --- | --- |
| [Can LLMs Generate Novel Research Ideas?](https://arxiv.org/abs/2409.04109) ([implementation and review scores](https://github.com/NoviScl/AI-Researcher)) (`measured_human_small_sample`) | 49 NLP researchers each wrote one proposal within a 10-day window; 79 researchers blind-reviewed 49 ideas per condition (298 reviews; 24 reviewers also wrote). Topic distribution and style were standardized across human and AI conditions. | At the idea level (49 proposals, 1–10 scale), human ideas averaged **4.86 ± 1.26 novelty**, **4.56 ± 1.16 excitement**, **6.53 ± 1.50 feasibility**, **5.10 ± 1.14 expected effectiveness**, and **4.69 ± 1.16 overall**. | This is a direct human ideation comparison, not an execution or publication baseline. The authors report that many proposals were generated on the spot and were roughly median-level relative to the writers’ past ideas (self-reported average around the 43rd percentile); do not generalize the scores to full research agents. |
