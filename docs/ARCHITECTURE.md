...
| Workspace status facade | `xscientist/workspace_status.py` | Semver-managed JSON projection | One read-only view over run state, worktree safety, scientific gates, generated-view freshness, and bounded next actions |
| Workspace history facade | `xscientist/workspace_history.py` | Semver-managed public API | Small payload-free list/show/diff/save/preview/reversal API over Research VCS; no persistence logic |
...
The native Research VCS is deliberately independent of GitHub and the HTTP
service. Each scientific project initializes its own typed object and commit
graph; compact research state and ARA hashes are versioned while large
immutable payloads remain in `.ara-store/`. The current Git adapter uses a
...
compare semantic changes, save selected or policy-eligible paths, preview one
exact reversal, and append a `revert` checkpoint only after explicit
confirmation. `workspace_status.py` reuses the same worktree classifier and
closure ladder, so status, bundling, switching, and rollback cannot silently
disagree about whether work is clean or scientifically promotable. Advanced
callers keep using `ResearchRepository` and the full protocol without
translation loss.
